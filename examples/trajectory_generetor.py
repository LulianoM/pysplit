...
def generate_bulktraj(basename, hysplit_working, output_dir, meteo_dir, years,
    ...
    """
    Gera uma sequência de trajetórias dentro de um determinado período de tempo.

    Execute a sequência em massa de simulações HYSPLIT em um determinado momento e em diferentes
    altitudes (provavelmente em metros acima do nível do solo). Uso semanal ou
    dados semestrais com o formato de nome de arquivo *mon*YY*# ou *mon*YYYY*#.
    Os resultados são gravados em ``output_dir``.

    Isso não define a saída meteorológica ao longo da trajetória - edite SETUP.CFG
    no diretório de trabalho HYSPLIT ou na GUI HYSPLIT4 para refletir
    variáveis de saída desejadas.
    
    Caminhos absolutos fortemente recomendados em caminhos relativos.
